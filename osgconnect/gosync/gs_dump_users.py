...
def get_users(options, client, db):
    client.get_group_members(self.groupid(self.topgroup))
    headers, response = self.client.get_group_members(self.groupid(self.topgroup))
    members = [member for member in response['members'] if member]
    ...
    for member in members:
        ...
        if '@' in username:
            logging.error("username has @ in it, skipping\n")
            continue
        if username in db:
    ...
    for member in selected:
        ...
        if member['status'] != 'active':
            continue    
        try:
            user_profile = self.client.get_user_profile(username)[1]
        except socket.timeout:
            # if we time out, pause and resume, skipping current
            logging.error("Socket timeed out. Waiting for 5 seconds. User %s was skipped" % username)
            time.sleep(5)
            continue
        if user_profile.has_key('credentials'):
