...
from gosync.connect_db.connect_db import connect_db
...
class globus_db_hybrid(globus_db):
    ...
    def get_groups(self,
        ...
        for top_child in self.group_tree["children"]:
            # if self.check_filters(top_child["name"], filters_top_group):
            #     continue
            # group_info = self.get_group_info(top_child)
            # if self.check_new_members(top_child["name"],
            #                           group_info["num_members"]):
            #     continue
            # group_info_tree[self.group_tree["name"]][top_child["name"]] = {
            #     "info": group_info}
            ...
            for child in top_child["children"]:
                # if self.check_filters(child["name"], filters_names):
                #     continue
                ...
                if self.process_child(group_info_tree, child, filters_names,
                                      top_group=top_child["name"]):
                    continue
        self.groups = group_info_tree
        return self.groups

    def process_child(self, group_info_tree, child, filters_group=None,
                      top_group="connect"):
        ...
        if self.check_new_members(child["name"],
                                  group_info["num_members"]):
            return True
        if top_group == "connect":
            group_info_tree[self.group_tree["name"]][child["name"]] = {
                "info": group_info}
        else:
            group_info_tree[self.group_tree["name"]][top_group["name"]][
                child["name"]] = group_info
        return False
    ...
    def check_group_membership_changes(self, group_name, globus_member_count):
        connect_member_count = self.connect_db.check_member_count(group_name)
        return (connect_member_count == globus_member_count)

    def get_globus_group_members(self, group_name=None):
        self.members = {}
        if self.groups is None:
            self.get_groups(filters_names=group_name)
        group_id = self.groups["connect"]["info"]["id"]
        members = self.wrapped_get_group_members(group_id=group_id)
        self.members["connect"] = members
        if group_name is None and self.only_update_members:
            return self.members
        for top_child in self.groups["connect"].keys():
            if (group_name is not None and
                group_name not in self.groups[
                    "connect"][top_child].keys()):
                continue
            for child in self.groups["connect"][top_child].keys():
                group_id = self.groups["connect"][top_child][child]["id"]
                members = self.wrapped_get_group_members(group_id=group_id)
