...
def get_sn_logical_ids_map(outfilename = "disk_sn_to_logical_id.txt"):
    ...
    with open(outfilename, "wt") as f:
        ...
        for d in glob.glob("/dev/sd*"):
            ...
            re_serial_num = re.findall("Serial number(.*)", output, re.MULTILINE)
            if re_serial_num:
                serial_num = re_serial_num[0].lstrip(":").replace(" ", "")
            else:
                continue
            re_logical_id = re.findall("Logical Unit id(.*)", output, re.MULTILINE)
            if re_logical_id:
...
def get_sn_label_map(infilename = "disk-map-v3"):
    ...
    with open(infilename, "rt") as infile:
        for l in infile:
            l = l.strip("\n")
            label, sn, mount_point = l.split(" ")
            sn_to_label[sn] = label
    return sn_to_label

def write_sn_label_logical_id_map(sn_to_label, sn_to_lid, outfilename = "disk-map-v4"):
    """
    Writing out mapping
    ...
    with open("disk-map-v4", "wt") as outfile:
        for sn, label in sorted(sn_to_label.items(), key=operator.itemgetter(1)):
